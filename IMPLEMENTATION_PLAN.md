...
**Status:** All phases implemented; release hardening items listed below remain
**Date:** 2026-09-13 (status updated 2026-09-15)
...
| 3 — Generic scraping engine | Done | Preset validation and status gating, user-host scoping, acknowledgement, limits, robots, challenge/redirect stops, transforms/types, `next_link`/`page_parameter`/`api_cursor`, authorized API credentials, test mode, staged datasets. |
| 4 — Embedded WebView and Scrape Studio | Done | Native incognito child WebView, allow-listed bridge, host-enforced navigation scope, repeated-item/element/next-page pickers with relative selectors, 10-record test, derived custom presets, multi-page rendered runs, login/challenge stops (D7). Verified in the running desktop app. `infinite_scroll` and `detail_links` pagination are not implemented. |
| 5 — Deterministic matching | Done | Normalization, multi-pass blocking with caps, evidence, guards, decisions, reproducible output. |
| 6 — Clustering, canonicalization, exports | Done | Constrained union-find with bridge rejection and locks, survivor rules, field provenance, four exports with hashes and final/unresolved marking, cluster and review actions in the audit. |
| 7 — Review queue and UI | Done | 5-step tab, preview staleness, optimistic versions, undo, shortcuts, masking, split/lock/unlock groups, responsive layout. |
| 8 — Quality and performance | Mostly done | 79 tests (64 Python, 6 TS, 9 Rust): preset fixture health for every bundled preset, a packaged-service smoke test, and CI. Measured 48.5k rows in about 3.4 s. Property-based tests and automated accessibility checks are still to do. |
...
| 10 — Secure updates and release | Done (unpublished) | PyInstaller service, NSIS installer with bundled resources, signed updater artifacts, GitHub-pinned update checks with approval and an active-job block, CI and tag-driven release workflows (SHA256SUMS, SBOM, beta channel). The first public release still needs a GitHub repository, release-environment secrets, and optional Authenticode code signing. |
